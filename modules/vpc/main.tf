...
resource "aws_internet_gateway" "gw" {
  ...
  tags = {
    Name = "eks-internet-gateway-${var.cluster_name}"
  }
}
...
resource "aws_nat_gateway" "gw" {
  ...
  tags = {
    Name = "eks-nat_Gateway-${count.index + 1}-${var.cluster_name}"
  }
}
...
resource "aws_route_table" "internet-route" {
  ...
  depends_on = [ aws_vpc.main ]
  tags  = {
      Name = "eks-public_route_table-${var.cluster_name}"
      state = "public"
  }
}
  ...
  depends_on = [ aws_vpc.main ]
  tags  = {
      Name = "eks-nat_route_table-${count.index + 1}-${var.cluster_name}"
      state = "public"
  } 
}
